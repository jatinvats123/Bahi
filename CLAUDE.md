...
- Phase 7: `scripts/demo-reset.ts` (`npm run demo:reset`: clean Bahi's sandbox data, seed the stage story, print stage commands), `src/lib/integrations/gmail/cursor.ts` (inbox cursor `data/inbox-cursor.json`), `src/instrumentation.ts` (IPv4-first DNS), `cliEnv()` in `transport.ts` (Swytchcode telemetry off). Docs: `docs/DEMO_RUNBOOK.md`, `docs/PITCH.md`, `docs/architecture.svg`.
...
- `npm run demo:reset [-- --dry | --clean-only]` (live: clean + seed the stage story; mock: resets the running server's world)
